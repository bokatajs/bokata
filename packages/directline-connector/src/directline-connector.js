...
class DirectlineConnector extends Connector {
  ...
  registerDefault() {
    this.container.registerConfiguration('directline', { log: true, channelId: 'emulator' }, false);
  }
  ...
  start() {
    ...
    server.get(`/directline/conversations/:conversationId/activities`, async (req, res) => {
      const watermark = req.query.watermark && req.query.watermark !== 'null' ? Number(req.query.watermark) : 0;
      this.log('debug', `GET /directline/conversations/:conversationId/activities`);
      const result = await this.controller.getActivities(req.params.conversationId, watermark);
      res.status(result.status).send(result.body);
    });

    server.post(`/directline/conversations/:conversationId/activities`, async (req, res) => {
      this.log('debug', `POST /directline/conversations/:conversationId/activities`);
      const result = await this.controller.addActivity(req.params.conversationId, req.body);
      res.status(result.status).send(result.body);
    });
    ...
    server.post(`/directline/conversations/:conversationId/upload`, async (req, res) => {
      this.log('debug', `POST /directline/conversations/:conversationId/upload`);
      ...
      form.parse(req, async (err, fields, files) => {
        if (err) {
          ...
          const activity = JSON.parse(fs.readFileSync(files.activity.path, 'utf-8'));
          activity.file = files.file;
          const result = await this.controller.addActivity(req.params.conversationId, activity);
    ...
    server.post(`/v3/directline/conversations/:conversationId/upload`, (req, res) => {
      this.log('debug', `POST /v3/directline/conversations/:conversationId/upload`);
      res.status(200).end();
    });

    server.get('/v3/directline/conversations/:conversationId/stream', (req, res) => {
      this.log('debug', 'GET /v3/directline/conversations/:conversationId/stream');
      res.status(200).end();
    });
    ...
    server.post('/v3/conversations/:conversationId/activities/:activityId', async (req, res) => {
      this.log('debug', 'POST /v3/conversations/:conversationId/activities/:activityId');
      const result = await this.controller.postActivityV3(req.params.conversationId, req.body);
      res.status(result.status).send(result.body);
    });
    ...
    server.get('/v3/conversations/:conversationId/activities/:activityId/members', (req, res) => {
      this.log('debug', 'GET /v3/conversations/:conversationId/activities/:activityId/members');
      res.status(200).end();
    });
    ...
    server.get('/v3/botstate/:channelId/conversations/:conversationId/users/:userId', (req, res) => {
      this.log('debug', 'GET /v3/botstate/:channelId/conversations/:conversationId/users/:userId');
      res.status(200).end();
    });
    ...
    server.post('/v3/botstate/:channelId/conversations/:conversationId', (req, res) => {
      this.log('debug', 'POST /v3/botstate/:channelId/conversations/:conversationId');
      res.status(200).end();
    });

    server.post('/v3/botstate/:channelId/conversations/:conversationId/users/:userId', (req, res) => {
      this.log('debug', 'POST /v3/botstate/:channelId/conversations/:conversationId/users/:userId');
      res.status(200).end();
    });
