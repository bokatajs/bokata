...
class Session {
  constructor(connector = {}, activity = {}) {
    ...
    this.channelId = this.activity.channelId || this.connector.settings.tag || 'emulator';
    this.conversation = {
    ...
    this.from = {
      ...
      name: process.env.BACKEND_NAME || this.connector.settings.tag || 'emulator',
    };
  ...
  async say(srcMessage, context) {
    ...
    if (typeof srcMessage === 'string') {
      ...
      if (context && context[localeDangle]) {
        message.text = context[localeDangle].getLocalized(context.locale || 'en', srcMessage);
      } else {
