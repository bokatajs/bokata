...
const { lowerNormalize, wordPunctTokenize, tokenize, normalize } = require('./base-fn');
...
class MarkovChain {
  ...
  getSortedChilds(tokens, avoidLongs = false) {
    ...
    for (let i = 0; i < childs.length; i += 1) {
      const values = Object.values(childs[i]).sort((a, b) => b.weight - a.weight);
      result.push(values);
    }
    return result;
  }
  ...
  predictNext(text, amount = 5) {
    const tokens = tokenize(normalize(text, this.ngrams.normalizer), this.ngrams.tokenizer);
  ...
  predictNextRandom(text) {
    const tokens =
      typeof text === 'string' ? tokenize(normalize(text, this.ngrams.normalizer), this.ngrams.tokenizer) : text;
  ...
  randomSentence(startText = '', maxLength = 30) {
    const tokens = tokenize(normalize(startText, this.ngrams.normalizer), this.ngrams.tokenizer);
    let word = '';
    while (word !== this.ngrams.endToken && tokens.length < maxLength) {
      word = this.predictNextRandom(tokens);
      tokens.push(word);
    }
    return tokens.filter((x) => x !== this.ngrams.startToken && x !== this.ngrams.endToken).join(' ');
  }
}
