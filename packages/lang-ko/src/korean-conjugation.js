  ...
  for (const word of words) {
    ...
    if (lastChar === '하') {
      expandedLast = [
        ...
        ...['ㅂ', 'ㅆ', 'ㄹ', 'ㄴ', 'ㅁ'].map((coda) => composeHangul('ㅎ', coda === 'ㅆ' ? 'ㅐ' : 'ㅏ', coda)),
        ...build('하', [13, 5, 10]),
        ...build('해', [1]),
        ...[...(isAdjective ? '합해히하' : '합해')],
      ];
    } else if (lastVowel === 'ㅗ' && lastCoda === ' ') {
      expandedLast = [].concat(build(lastChar, [13, 6, 3, 10]), buildNoPast(lastOnset, 'ㅗ'), [
        composeHangul(lastOnset, 'ㅘ'),
        composeHangul(lastOnset, 'ㅘ', 'ㅆ'),
        lastChar,
      ]);
    } else if (lastVowel === 'ㅜ' && lastCoda === ' ') {
      expandedLast = [].concat(build(lastChar, [13, 2, 6, 10]), buildNoPast(lastOnset, 'ㅜ'), [
        composeHangul(lastOnset, 'ㅝ'),
        composeHangul(lastOnset, 'ㅝ', 'ㅆ'),
        lastChar,
      ]);
    } else if (lastVowel === 'ㅡ' && lastCoda === ' ') {
      expandedLast = [].concat(build(lastChar, [6, 10]), buildNoPast(lastOnset, 'ㅡ'), [
        composeHangul(lastOnset, 'ㅝ'),
      ...
      expandedLast = [].concat(buildNoPast(lastOnset, 'ㅣ'), build(lastChar, [2, 6, 10]), [
      ...
      lastCoda === 'ㄹ' &&
      ((lastOnset === 'ㅁ' && lastVowel === 'ㅓ') || lastVowel === 'ㅡ' || lastVowel === 'ㅏ' || lastVowel === 'ㅜ')
    ) {
      expandedLast = [].concat(build(lastChar, [2, 7]), build(composeHangul(lastOnset, lastVowel), [6, 10, 12]), [
      ...
      expandedLast = [].concat(build(lastChar, [6, 10]), build(composeHangul(lastOnset, 'ㅜ'), [2, 4, 8, 9]), [
        composeHangul(lastOnset, 'ㅜ', 'ㄹ'),
        lastChar,
      ]);
      ...
      expandedLast = [].concat(build(composeHangul(lastOnset, lastVowel), [4, 11]), [
        composeHangul(lastOnset, lastVowel),
        lastChar,
      ]);
    } else if (lastVowel === 'ㅗ' && lastCoda === 'ㅎ') {
      expandedLast = [].concat(build(lastChar, [6, 10]), buildCommon(lastOnset, 'ㅗ'), [
        composeHangul(lastOnset, 'ㅘ'),
    ...
    if (initLast && lastChar === '르' && !hasCoda(initLast)) {
      ...
      const newInit =
        init.slice(0, -1) + composeHangul(lastInitCharDecomposed.onset, lastInitCharDecomposed.vowel, 'ㄹ');

      const o = lastCharDecomposed.onset;
      const conjugation = [].concat(build(lastChar, [6, 10]), buildNoPast(o, 'ㅡ'), [
        composeHangul(o, 'ㅝ'),
        composeHangul(o, 'ㅓ'),
        composeHangul(o, 'ㅏ'),
        composeHangul(o, 'ㅝ', 'ㅆ'),
