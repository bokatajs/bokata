...
class TestConnector extends Connector {
  ...
  async hear(line) {
    ...
    if (this.onHear) {
      ...
      if (pipeline) {
        this.container.runPipeline(pipeline, { message: line, channel: 'console', app: this.container.name }, this);
      } else {
  ...
  async parseTestScript(fileName) {
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (inputMatch) {
        ...
        const remoteTestScript = await this.parseTestScript(path.join(testPath, remoteFilePath));
