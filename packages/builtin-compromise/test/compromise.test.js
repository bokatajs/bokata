...
describe('Compromise Integration', () => {
  ...
  describe('English', () => {
    ...
    test('Compromise  English Ordinal', async () => {
      const actual = await extract('en', 'The second is the seventh prime number');
      const expected = [
        {
          start: 4,
          end: 9,
    ...
    test('Compromise  English Number Float', async () => {
      const actual = await extract('en', 'The number is one hundred and twelve point 5');
      ...
      const expected = [
        {
          ...
          resolution: {
            value: '2019-12-12T09:00:00.000',
          },
        },
      ];
      actual.edges[0].resolution.value = actual.edges[0].resolution.value.slice(0, 23);
      expect(actual.edges).toEqual(expected);
    });
