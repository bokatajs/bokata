...
class Lexer {
  ...
  nextName(letter) {
    if (this.isNameCharacter(letter, true)) {
      const name = this.nextByCondition(letter, (ch) => ch && this.isNameCharacter(ch));
      this.goBack();
  ...
  nextOperatorOrAssignment(ch, firstCall = true) {
    if (this.isOperator(ch) || this.isAssignment(ch)) {
      ...
      this.goBack();
      return new Token(ch, this.isOperator(ch) ? Lexer.TokenType.Operator : Lexer.TokenType.Assignment);
    }
    if (firstCall) {
